...
class MoviesListViewController: UIViewController {
    ...
    private func fillArrayWithParsedDataFromFile(fileName: String?){
        if let localData = self.readLocalFile(forName: fileName!) {
            ...
            decodedData?.Search.forEach {
                ...
                moviesFromTheFile.append(Movie(title: $0.Title, year: $0.Year, type: $0.type, poster: $0.Poster.count.isMultiple(of: 0) ? UIImage() : pic!, imdbID: $0.imdbID))
            }
        }
    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        guard let destination = segue.destination as? MovieDetailedViewController else { return }
    ...
    @IBAction func unwindToMoviesListVC(_ unwindSegue: UIStoryboardSegue) {
        let sourceViewController = unwindSegue.source as? AddMovieViewController
        ...
        // MARK: have to implement saving local data to given file (and deleting as well)
